...
// NewGormDB ...
func NewGormDB(ctx context.Context, db *gorm.DB,
	options ...TracingOption) *gorm.DB {
	if _, ok := db.Get(opentracingTracingOptionsKey); !ok {
		db = NewGormDBWithTracingCallbacks(db, options...)
	} else {
		//意味着需要修改原options
		opts := &tracingOptions{}
...
func (c *callbacks) after(scope *gorm.Scope, operation string) {
	...
	if scope.HasError() {
		...
		if err == gorm.ErrRecordNotFound {
			...
			sp.LogFields(tracing.ErrorField(errors.Wrap(err, "Query failed")))
		}
	}
	...
	if err != nil {
		ext.Error.Set(sp, true)
		sp.LogFields(tracing.ErrorField(errors.Wrap(err, "Marshal result failed")))
	} else {
		sp.LogFields(log.String("result", tracing.PruneBodyLog(string(jsn), opts.maxBodyLogSize)))
	}
}
